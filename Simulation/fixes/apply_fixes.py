...
def fix_center_of_mass_toroidal():
    ...
    if old_code in content:
        ...
        with open(file_path, 'w') as f:
            f.write(content)
        print(f"✓ Fixed center-of-mass toroidal wrapping in {file_path}")
        return True
    else:
        print(f"✗ Could not find old code pattern in {file_path}")
        return False
...
def fix_while_loop_wrapping():
    ...
    if old_code in content:
        ...
        with open(file_path, 'w') as f:
            f.write(content)
        print(f"✓ Optimized while-loop wrapping in {file_path}")
        return True
    else:
        print(f"✗ Could not find old code pattern in {file_path}")
        return False
...
def fix_speedup_baseline():
    ...
    if old_line in content:
        ...
        with open(file_path, 'w') as f:
            f.write(content)
        print(f"✓ Fixed speedup baseline in {file_path}")
        return True
    else:
        print(f"✗ Could not find old code pattern in {file_path}")
        return False
...
if __name__ == "__main__":
    ...
    print(f"\n{success_count}/{total_count} fixes applied successfully")

    if success_count == total_count:
        print("\n✓ All critical issues fixed!")
    else:
        print(f"\n⚠ {total_count - success_count} fixes failed - manual intervention may be needed")
