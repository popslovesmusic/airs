...
CommandRouter::CommandRouter()
    ...
    command_handlers["initialize_engine"] = [this](const json& p) { return handleInitializeEngine(p); };
...
json CommandRouter::handleListEngines(const json& params) {
    ...
    for (auto* engine : engine_list) {
        json engine_json = {
            ...
            {"engine_lifecycle_state", lifecycleStateToString(engine->lifecycle_state)}
        };
...
json CommandRouter::handleCreateEngine(const json& params) {
    ...
    json result = {
        ...
        {"num_nodes", num_nodes},
        {"engine_lifecycle_state", "CREATED"},
        {"R_c", R_c},
        {"kappa", kappa},
        {"gamma", gamma},
        {"dt", dt}
...
json CommandRouter::handleInitializeEngine(const json& params) {
    if (!params.contains("engine_id")) {
        return createErrorResponse("initialize_engine", "Missing engine_id", "MISSING_PARAMETER");
    }
    ...
    if (!instance) {
        return createErrorResponse("initialize_engine",
                                   "Engine not found: " + engine_id,
                                   "ENGINE_NOT_FOUND");
    }

    bool success = engine_manager->initializeEngine(engine_id);

    if (!success) {
        return createErrorResponse("initialize_engine",
                                   "Failed to initialize engine (invalid lifecycle state)",
    ...
    return createSuccessResponse("initialize_engine", result, 0);
}
...
json CommandRouter::handleRunMission(const json& params) {
    ...
    auto* instance = engine_manager->getEngine(engine_id);
    if (!instance) {
        return createErrorResponse("run_mission", "Engine not found: " + engine_id, "ENGINE_NOT_FOUND");
    }

    // Check lifecycle state before attempting to evolve
    if (instance->lifecycle_state != EngineLifecycleState::INITIALIZED) {
        return createErrorResponse("run_mission",
                                   "Invalid lifecycle state: expected INITIALIZED, got " + lifecycleStateToString(instance->lifecycle_state),
                                   "INVALID_LIFECYCLE_STATE");
    }
...
json CommandRouter::handleGetMetrics(const json& params) {
    ...
    if (instance) {
        result["engine_lifecycle_state"] = lifecycleStateToString(instance->lifecycle_state);
    }
...
json CommandRouter::handleGetState(const json& params) {
    ...
    auto* instance = engine_manager->getEngine(engine_id);
    if (!instance) {
        return createErrorResponse("get_state", "Engine not found: " + engine_id, "ENGINE_NOT_FOUND");
    }

    // Check lifecycle state before attempting to get state
    if (instance->lifecycle_state != EngineLifecycleState::EVOLVED) {
        return createErrorResponse("get_state",
                                   "Invalid lifecycle state: expected EVOLVED, got " + lifecycleStateToString(instance->lifecycle_state),
                                   "INVALID_LIFECYCLE_STATE");
    }

    if (!engine_manager->getAllNodeStates(engine_id, psi_real, psi_imag, phi)) {
        return createErrorResponse("get_state", "Failed to extract state (wrong engine type)", "STATE_EXTRACTION_FAILED");
    }
...
json CommandRouter::handleGetSatpState(const json& params) {
    ...
    auto* instance = engine_manager->getEngine(engine_id);
    if (!instance) {
        return createErrorResponse("get_satp_state", "Engine not found: " + engine_id, "ENGINE_NOT_FOUND");
    }

    // Check lifecycle state before attempting to get state
    if (instance->lifecycle_state != EngineLifecycleState::EVOLVED) {
        return createErrorResponse("get_satp_state",
                                   "Invalid lifecycle state: expected EVOLVED, got " + lifecycleStateToString(instance->lifecycle_state),
                                   "INVALID_LIFECYCLE_STATE");
    }
    ...
    if (!engine_manager->getSatpState(engine_id, phi, phi_dot, h, h_dot)) {
        return createErrorResponse("get_satp_state", "Failed to extract SATP state (wrong engine type)", "STATE_EXTRACTION_FAILED");
    }
