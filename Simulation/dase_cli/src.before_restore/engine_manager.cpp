...
bool EngineManager::initializeEngine(const std::string& engine_id) {
    auto* instance = getEngine(engine_id);
    if (!instance || !instance->engine_handle) {
        return false;
    }

    // Check precondition: must be in CREATED state
    if (instance->lifecycle_state != EngineLifecycleState::CREATED) {
        return false;
    }

    // Initialize internal state (no-op for now, engines are initialized on creation)
    // This is a placeholder for future initialization logic

    // Transition to INITIALIZED state
    instance->lifecycle_state = EngineLifecycleState::INITIALIZED;
...
bool EngineManager::runMission(const std::string& engine_id, int num_steps, int iterations_per_node) {
    ...
    // Check precondition: must be in INITIALIZED state
    ...
    try {
        ...
        // Transition to EVOLVED state
        instance->lifecycle_state = EngineLifecycleState::EVOLVED;
...
bool EngineManager::getAllNodeStates(const std::string& engine_id,
    ...
    // Check precondition: must be in EVOLVED state
