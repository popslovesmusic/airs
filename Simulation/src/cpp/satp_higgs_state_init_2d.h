/**
 * SATP+Higgs State Initialization - 2D Profiles
 *
 * Initialization functions for φ and h fields on 2D toroidal lattice including:
 * - Higgs VEV baseline
 * - Circular Gaussian pulses
 * - Plane waves
 * - 2D source configurations
 */

#pragma once

#include "satp_higgs_engine_2d.h"
#include <algorithm>
#include <cmath>
#include <random>
#include <string>
...
inline constexpr double MIN_SIGMA_SATP_2D = 1.0e-9;

struct GaussianProfile2DParams {
    double amplitude;
    double center_x, center_y;
    double sigma_x, sigma_y;  // Elliptical Gaussian
    bool set_velocity;
    double velocity_amplitude;
    std::string mode;  // "overwrite", "add", "blend"
    double beta;

    GaussianProfile2DParams()
        : amplitude(1.0), center_x(0.0), center_y(0.0),
          sigma_x(1.0), sigma_y(1.0),
          set_velocity(false), velocity_amplitude(0.0),
          mode("overwrite"), beta(1.0) {}
};

class SATPHiggsStateInit2D {
public:
    // Initialize to Higgs vacuum (VEV baseline)
    static void initVacuum(SATPHiggsEngine2D& engine) {
    ...
    // Initialize φ field with circular Gaussian
    static void initPhiCircularGaussian(SATPHiggsEngine2D& engine,
                                         double amplitude,
                                         double center_x, double center_y,
                                         double sigma,
                                         const std::string& mode = "overwrite") {
        GaussianProfile2DParams params;
    ...
    // Initialize φ field with elliptical Gaussian
    static void initPhiGaussian(SATPHiggsEngine2D& engine,
                                const GaussianProfile2DParams& params) {
        auto& nodes = engine.getNodesMutable();
        size_t N_x = engine.getNx();
        size_t N_y = engine.getNy();
        double dx = engine.getDx();

        const double sigma_x = std::max(params.sigma_x, MIN_SIGMA_SATP_2D);
        const double sigma_y = std::max(params.sigma_y, MIN_SIGMA_SATP_2D);
        ...
        for (size_t y = 0; y < N_y; ++y) {
            for (size_t x = 0; x < N_x; ++x) {
                size_t idx = engine.getIndex(x, y);
    ...
    // Initialize h field with circular Gaussian perturbation around VEV
    static void initHiggsCircularGaussian(SATPHiggsEngine2D& engine,
                                          double amplitude,
                                          double center_x, double center_y,
                                          double sigma,
                                          const std::string& mode = "overwrite") {
    ...
    static void initHiggsGaussian(SATPHiggsEngine2D& engine,
                                   const GaussianProfile2DParams& params) {
        auto& nodes = engine.getNodesMutable();
        size_t N_x = engine.getNx();
        size_t N_y = engine.getNy();
    ...
    static void initUniform(SATPHiggsEngine2D& engine,
    ...
    static void addRandomPerturbation(SATPHiggsEngine2D& engine,
    ...
    // Reset to vacuum
    static void reset(SATPHiggsEngine2D& engine) {
        engine.reset();
    }
};
